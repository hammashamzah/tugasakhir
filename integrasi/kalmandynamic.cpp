...
void KalmanDynamic::processDataCurrent(QList<Player> current) {
    previousData.clear();
    previousData = currentData;
    currentData.clear();
    currentData = current;
    sizeCurrent = currentData.size();
    sizePrevious = previousData.size();
    accum_kalmanobj();
}
    ...
    for (int i = 0; i < sizeCurrent; i++) {
        for (int j = 0; j < sizePrevious; j++) {
            ...
            if (idx >= sizePrevious) {
                ...
                measuring[idxm] = Player(currentData.at(i).id, currentData.at(i).framePosition, currentData.at(i).camera, currentData.at(i).pos,
                                         Point2f(vx_measurement_buffer, vy_measurement_buffer), Point2f(ax_measurement_buffer, ay_measurement_buffer));
                track_ind2Dmotion(Player(currentData.at(i).id, currentData.at(i).framePosition, currentData.at(i).camera, currentData.at(i).pos,
                                         Point2f(vx_measurement_buffer, vy_measurement_buffer), Point2f(ax_measurement_buffer, ay_measurement_buffer)),
                                  priory_posp, priory_velsp, priory_accp, post_posp, post_velsp, post_accp);
