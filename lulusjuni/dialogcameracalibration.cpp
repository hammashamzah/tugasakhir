...
Mat Settings::nextImage()
{
    ...
    if( atImageList < 2 )
        result =  (cv::imread("new1"), CV_LOAD_IMAGE_COLOR);
...
void CameraCalibrationProcess(QList<Mat> frame_selected)
{
    Settings s;
    ...
    for(int i = 0;;++i)
    {
      ...
      static int frameke=0;
      frameke++;
      if (frameke<frame_selected.size())
      {
              view = frame_selected.at(frameke);
          qDebug()<<"nah ";
          kosong = false;
      }
      else
      {
          view = Mat::zeros(3,3, CV_8U);
          qDebug()<<"looh";
          kosong = true;
      }
      ...
      if(kosong)          // If no more images then run calibration, save and stop loop.
      {
        ...
        if( mode == CALIBRATED && s.showUndistorsed )
        {
            ...
            qDebug()<<"generate undistort image";
        }
    ...
    if( s.inputType == Settings::IMAGE_LIST && s.showUndistorsed )
    {
        ...
        for(int i = 0; i < frame_selected.size(); i++ )
        {
            view = frame_selected.at(i);
            if(view.empty())
                continue;
...
void DialogCameraCalibration::on_pushButton_runCalibration_clicked()
{
    CameraCalibrationProcess(frame_selected);
}
