...
void VideoProcessor::processSingleFrame()
{
	...
	if (mode == 0) {
		...
		contours.clear();
        contours = blob_detector.getContours();
	...
	for (int i = 0; i < points.size(); i++) {
        outputData.append(Player((int)this->getCurrentFrame(), 0, Point2f(points[i].x, points[i].y), contourArea(contours[i])));
        qDebug() << i << ": " << outputData.at(outputData.size() - 1).blobSize;
	}
