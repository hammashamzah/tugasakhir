...
/*void DataWindow::updatePosition()
{
	...
	frameNumber++;
	if (frameNumber < tempPlayersData.size()) {
...
void DataWindow::processData(QList<Player> dataPlayer) {
	//add data to datalogger
    for(int i = 0; i < dataPlayer.size(); i++){
		qDebug() << dataPlayer.at(i).id << " " << dataPlayer.at(i).blobSize;
	}
...
/*QVector<QList<Player> > DataWindow::setRandomQList()
{
	...
	for (int i = 0; i < 22; ++i)
	{
		for (int j = 0; j < tempPlayersData.size(); ++j)
		{
			playerUnit.id = i;
...
{
	frameNumber = value * tempPlayersData.size() / 100;
	ui->label_playback_time->setText("Frame " + QString::number(frameNumber));
//	updatePosition();
}
