...
CameraObjectProcessor::CameraObjectProcessor(int cameraNumber, QList<QPoint> trapeziumCoordinate, double pixelThreshold)
{
    ...
    searchingdata = new SeachPlayerData();
    IssetCurrent = false;
}
...
void CameraObjectProcessor::getDataCurrent(QList<Player> current){
    curr.clear();
    curr = current;
    IssetCurrent = true;
}
void CameraObjectProcessor::camAssociate(int data_before,int Fr,QList<Player> predict,QList<Player> prev){
    int i,j;
    if(IssetCurrent){
        pred.clear();
        ...
        if(frames == 1){
            ...
            if(!previous.isEmpty()){
                ...
                emit sendOcclusion(pot_Occlusion);
            }
        }
       emit updateMatrices(Associate);
       emit sendLostFound(indicatedLostFound);
   }
   IssetCurrent = false;
}
